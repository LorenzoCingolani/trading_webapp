...
def compute_pnls(trades_needed: pd.Series, current_pos: pd.Series, fm: pd.DataFrame,
                 px_closes: pd.Series, px_closes_prev: pd.Series) -> tuple[list[float], list[float]]:
...
def compute_trades(px_closes: pd.Series, px_closes_prev: pd.Series, std_dev: pd.Series, alpha_forecast: pd.Series,
                   PDM: pd.Series, alpha_current_pos: pd.Series, fm: pd.DataFrame, cash_vol_daily: list) -> tuple:
    ...
    return (one_perc_change, block_value, price_volatility, cash_vol_daily, std_dev, icv, ivv, vol_scalar,
            pos_contracts, trades_needed, target_pos, daily_instrument_pnls, daily_current_pnls)


def framework_main(combinedForecastCSVPath: str, csv_dictionary: dict, PDM: pd.Series, date_format: str, aum: float,
                   is_markov: bool = False, std_dev_days: int = 20) -> pd.DataFrame:
    aums = []
    ...
    for instrument in product_list:
        df = csv_dictionary[instrument]
        if 'Instrument_Weights' not in df.columns:
            raise ValueError(f"Missing Instrument_Weights in input data for {instrument}")
        row = df.iloc[0].copy()
        row['INSTRUMENT'] = instrument
        fm_rows.append(row)

        try:
            all_px_closes[instrument] = get_col_data(df, 'PX_CLOSE_1D', 'Date', date_format)
            all_std_dev[instrument] = get_col_data(df, 'st_dev', 'Date', date_format)

            forecast_path = os.path.join(combinedForecastCSVPath, f"{instrument}.csv")
            if is_markov:
                all_alpha_forecast[instrument] = get_col_data(df, 'MarkovAdjFinalForecast', 'Date', date_format)
            else:
                forecast_df = pd.read_csv(forecast_path)
                all_alpha_forecast[instrument] = get_col_data(forecast_df, 'FinalForecast', 'Date', date_format)

        except Exception as ex:
            print(f"Skipping {instrument} due to error: {ex}")
    ...
    fm = pd.DataFrame(fm_rows).set_index('INSTRUMENT')
    ...
    new_cols = [f'{col}_{feature}' for col in val_cols for feature in feature_labels]

    trades_df = pd.DataFrame.from_records(trades, index=alpha_forecast_df.index, columns=new_cols)
    trades_df['AUM'] = aums
    return trades_df
