...
def get_col_data(file_path, col_name,date_col='Date',date_format='%Y-%m-%d'):
...
def main_frame(fm, read_fold, input_folder, PDM, date_format, aum, is_markov=False, std_dev_days=20):
    aums = []
    ...
    for instrument in fm['Instruments']:
        try:
            if is_markov:
                all_alpha_forecast[instrument] = get_col_data(os.path.join(read_fold, f'{instrument}.csv'), 'MarkovAdjFinalForecast')
                all_std_dev[instrument] = all_px_closes[instrument].rolling(std_dev_days).std()
            else:
                all_alpha_forecast[instrument] = get_col_data(os.path.join(read_fold, f'{instrument}.csv'), 'FinalForecast')
            all_px_closes[instrument] = get_col_data(os.path.join(input_folder, f'{instrument}.csv'), 'PX_CLOSE_1D', 'Date', date_format=date_format)
            # Compute sta
            # all_std_dev[instrument] = all_px_closes[instrument].rolling(std_dev_days).std()
            all_std_dev[instrument] = get_col_data(os.path.join(input_folder, f'{instrument}.csv'), 'st_dev', 'Date', date_format=date_format)
            # all_std_dev[instrument] = get_col_data(os.path.join(input_folder, f'{instrument}.csv'), 'st_dev', 'Date', date_format=date_format)
    ...
    print(trades.head())
    csv_path = os.path.join(read_fold, f"trades_{'markov' if is_markov else 'alpha'}.csv")
...
if __name__ == '__main__':
    ...
    input_folder = './Files/all_in_1year/portfolio1'
    read_fold='./Files/all_out_1year/Model_output_combinedForecast/'
    date_format="%d/%m/%Y"
    ...
    fm=pd.read_csv(framework_input_file)
    main_frame(fm, read_fold, input_folder,  PDM, date_format, aum, is_markov=False)
