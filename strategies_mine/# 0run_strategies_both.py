#!/usr/bin/env python
"""Main entry point for portfolio simulation with EWMA and Carry strategies."""
import os
import sys
import numpy as np
import pandas as pd
...
def save_strategy_results(
    strategy_name, state, nav, aligned, scaffold, pdm, instruments, out_dir
):
    ...
    print(f"\n📊 {strategy_name.upper()} PORTFOLIO SUMMARY")
    print("-" * 60)
    print(f"PDM:                  {pdm:.3f}")
    ...
    for inst, df in aligned.items():
        sc = scaffold[inst]
        st = state[inst]
        forecast_col = st.get("forecast", st.get("vol_scaler"))
        out = pd.DataFrame(
            {
                "Date": df["Date"],
                "PX_CLOSE_1D": sc["px"],
        ...
        out_path = os.path.join(out_dir, f"{inst}_{strategy_name.upper()}_timeseries.csv")
        out.to_csv(out_path, index=False)
        print(f"✅ Saved {strategy_name} {inst} → {out_path}")
    ...
    summary_path = os.path.join(
        out_dir, f"PORTFOLIO_{strategy_name.upper()}_summary.csv"
    )
    summary.to_csv(summary_path, index=False)
    print(f"✅ Saved {strategy_name} summary → {summary_path}")

    return summary


def main():
    """Run portfolio simulation with both strategies."""
    print("\n" + "=" * 60)
    print("MULTI-STRATEGY PORTFOLIO FRAMEWORK")
    print("=" * 60)

    # ================== LOAD DATA ==================
    print("\n[1/7] Loading instrument data...")
    combo = {}
    inputs = {}
    ...
    # ================== ALIGN DATES ==================
    print("\n[2/7] Aligning dates across instruments...")
    dates = None
    ...
    # ================== COMPUTE PDM ==================
    print("\n[3/7] Computing PDM from price correlations...")
    aligned_px = {
    ...
    print("\n[4/7] Building sizing scaffold per instrument...")
    scaffold = {}
    for inst, df in aligned.items():
        point_value = float(
    ...
    # ================== RUN EWMA STRATEGY ==================
    print("\n[5/7] Running EWMA strategy simulation...")
    ...
    # ================== RUN CARRY STRATEGY ==================
    print("\n[6/7] Running Carry strategy simulation...")
    ...
    summary_ewma = save_strategy_results(
        "ewma", ewma_state, ewma_nav, aligned, scaffold, pdm, INSTRUMENTS, OUT_DIR
    )

    summary_carry = save_strategy_results(
        "carry", carry_state, carry_nav, aligned, scaffold, pdm, INSTRUMENTS, OUT_DIR
    )

    # Combined summary
    combined_summary = pd.concat([summary_ewma, summary_carry], ignore_index=True)
    combined_path = os.path.join(OUT_DIR, "PORTFOLIO_COMBINED_summary.csv")
    combined_summary.to_csv(combined_path, index=False)
    print(f"✅ Saved combined summary → {combined_path}")

    print("\n" + "=" * 60)
    print("✅ EXECUTION COMPLETE - Both strategies ran successfully!")
    print("=" * 60 + "\n")
