...
if __name__ == "__main__":
    ...
    # Test case with synthetic data to confirm cash-only EWMA works
    print("\n=== Test Case: Cash-only EWMA with Synthetic Data ===")
    # test_prices = [100, 101, 102, 103, 104, 105, 106, 107, 108, 109] * 10  # 100 data points
    # test_df = pd.DataFrame({
    #     'Date': pd.date_range('2020-01-01', periods=len(test_prices)),
    #     'PX_CLOSE_1D': test_prices
    # })
    test_df = pd.read_csv(r"C:\Users\loci_\Desktop\trading_webapp\DATA\ETHUSDT_daily.csv")
    test_df['Date'] = pd.to_datetime(test_df['date'], dayfirst=True, errors='coerce')
    test_df["PX_CLOSE_1D"] = test_df["close"].astype(float)
    test_df = test_df[['Date', 'PX_CLOSE_1D']]



    test_out = compute_all_ewma(test_df, FORECAST_SCALERS, cap=CAP)
    print(f"Test DataFrame has {len(test_out)} rows.")
    for (fast, slow), _ in FORECAST_SCALERS.items():
        col = f"ewma_{fast}d_{slow}d_forecast"
        if col in test_out.columns:
            last_forecast = test_out[col].dropna().iloc[-1] if not test_out[col].dropna().empty else np.nan
            print(f"{fast}d_{slow}d last forecast: {last_forecast:.3f}")
        else:
            print(f"ERROR: Missing forecast column {col}")
    print("✅ Test case completed successfully.")
    # save ouput

    test_out_path = os.path.join(OUT_DIR, f"{OUT_PREFIX}_EWMA_TEST.csv")
    test_out.to_csv(test_out_path, index=False)
    print(f"✅ Saved test output: {test_out_path}")
