"""EWMA (Exponential Weighted Moving Average) strategy."""
import sys
import os
import numpy as np
import pandas as pd
...
from common.utils import TRADING_DAYS, ANNUAL_TARGET_VOL, ALPHA, ewma_stdev_net, pick_col


class EWMAStrategy:
    """EWMA-based position sizing and forecasting strategy."""
    ...
    def compute_positions(self) -> Tuple[Dict, np.ndarray]:
        """
        Compute positions for all instruments across time.
        ...
        # Pre-compute EWMA momentum forecasts for all instruments
        for inst, df in self.aligned_data.items():
            ...
            if px_col:
                ...
                # Simple EWMA-based momentum: standardized returns
                stdev = ewma_stdev_net(px, ALPHA)
                returns = px.pct_change() * 100.0
                momentum = returns / (stdev.replace(0.0, np.nan) ** 2)
                forecasts[inst] = momentum.fillna(0.0).clip(-20.0, 20.0)
            else:
