...
def analyze_single_week_day(week_high, week_low, week_date, week_close,
    ...
    for low_daily, high_daily, date_d in zip(daily_lows, daily_highs, daily_dates):
        ...
        if any(triggering):
            ...
            # in case of stop loss for unit bought @ 23.6% , we have to get sub_bucket_df.iloc[1,3]
            stop_loss_level_23_6 = sub_bucket_df.iloc[1, 3]
            # stop loss for unit bought @38.2% is sub_bucket_df.iloc[1, 3] and sub_bucket_df.iloc[1, 4]
            stop_loss_level_38_2 = sub_bucket_df.iloc[1, 3:5].tolist()
            print("stop_loss_level_23_6:", stop_loss_level_23_6)
            print("stop_loss_level_38_2:", stop_loss_level_38_2)
            if high_daily >= stop_loss_level_23_6:
                print('take profit at 23.6%')
            if low_daily <=  stop_loss_level_23_6:
                print('stop loss at 23.6%')
    # we have main bucket
    # we have sub bucket
    # we have triggering information in main bucket


    
    return main_bucket_df
