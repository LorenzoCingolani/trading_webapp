...
def analyze_single_week_day(week_high, week_low, week_date, week_close,
    ...
    # Example: Only some levels are buy levels
    ...
    for low_daily, high_daily, date_d in zip(daily_lows, daily_highs, daily_dates):
        ...
        # Initialize stop loss and take profit columns for all levels
        stop_loss_col = []
        take_profit_col = []
        ...
        for i in range(len(main_bucket_df)):
            ...
            # Only check stop loss if unit was ever bought (non-zero unit)
            if units > 0:
                try:
