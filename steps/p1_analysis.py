    ...
    for inst_idx, (ins_name, data) in enumerate(instrument_items, start=1):
        ...
        if 'ewma' in ModelsList:
            ...
            try:
                ...
                if not passed_ewma:
                    st.warning(f"EWMA generated no forecasts for {Inst_name}; all speeds failed the cost filter.")
            except Exception as ex:
                st.warning(f'EWMA Strategy failed for {Inst_name}: {ex}')
