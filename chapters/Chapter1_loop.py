...
if __name__ == '__main__':
    ...
    folder_path ='C:/Users/User/trading-strategy-/Files/all_in_1year/instrument_updated_close'
    for file in os.listdir(folder_path):
        if file.endswith('.csv'):
            data = pd_readcsv(folder_path + '/' + file)
            data = data.dropna()
            ...
            data_all_stuff.to_csv(f'output_data/{file[:-4]}_data.csv',index=None)
            #C:\Users\User\trading-strategy-\Files\all_in_1year\instrument_updated_close
            ...
            data_summary = pd.DataFrame([calculate_stats(perc_return, at_frequency=MONTH)])
            data_summary.to_csv(f'output_data{file[:-4]}_summary.csv',index=None)
